...
**5. "결과가 기각이어도, 파이프라인을 처음부터 설계·구현한 경험은 남는다."**
영상 컷 분할·투수 검출·관절 추출·좌투 미러링까지 **직접 만든 데이터 파이프라인**이었습니다. 최종 결론은 "채택 안 함"이었지만, 원시 영상에서 정형화된 feature를 만들어내는 엔드투엔드 과정 자체가 이 프로젝트에서 가장 크게 남은 실무 역량입니다.
