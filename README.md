...
야구 중계에서 해설자는 "오늘 저 투수 공이 좋다"고 말합니다. 하지만 그건 **경기가 끝난 뒤에야** 아는 사실입니다.
이 프로젝트는 그 판단을 **경기 초반 15구 시점으로 앞당기는 것**을 목표로 했습니다.

- **핵심 철학**: "이 투수가 **누구인가**(통산 성적)"가 아니라 "**오늘 어떤 상태인가**(컨디션)"를 예측
...
Statcast에 **없는 정보**(신체 릴리스 자세)를 넣으면 이길 수 있다고 봤습니다.
9초 클립 → 컷 분할 → 투수 검출 → 릴리스 프레임 → MediaPipe 14관절 → **각도 9종**(arm_slot, shoulder_tilt, separation 등)까지 직접 설계했습니다.
(좌투 미러링 · 어깨너비 정규화 · 윈저라이징 이상치 처리 포함)
...
**1. "모델을 개선하기 전에, 예측 가능한 문제인지부터 확인한다."**
초기 타겟(wOBA)은 R² 0.005로 사실상 예측 불가였습니다. 하이퍼파라미터를 아무리 만져도 오르지 않았을 것입니다.
타겟을 whiff%로 재정의하자 **9배 개선**됐습니다. 성능의 대부분은 모델이 아니라 **문제 정의**에서 나왔습니다.

**2. "가설이 틀렸다는 것도 결과다."**
이 프로젝트의 두 핵심 아이디어(**delta feature**, **영상 생체역학 융합**)는 **둘 다 기각**됐습니다.
그대로 넣고 "SHAP 상위권에 올랐다"고 포장할 수도 있었지만, **30 seed paired t-test**로 검증해 `p ≈ 9.6×10⁻⁷`로 **유의하게 나빠진다**는 것을 확인하고 배제했습니다.
특히 영상은 "무의미하다"에서 끝내지 않고 **왜 안 됐는지**(상관 약함 / 중복 아님 / 정지 1프레임의 한계)까지 3단계로 규명했습니다.

**3. "SHAP 중요도는 예측 기여가 아니다."**
피처 중요도만 보고 채택했다면 영상 피처를 넣었을 것입니다. **중요도 랭킹과 실제 성능 기여는 다르다**는 것을 직접 실험으로 확인한 것이 가장 값진 교훈이었습니다.

**4. "평균 성능이 아니라, 언제 쓸 수 있는 모델인지를 말한다."**
R² 0.034를 그대로 보고하는 대신, **파워형 투수(R² +0.018 / AUC 0.63)** 라는 실용 범위를 찾아냈습니다.
