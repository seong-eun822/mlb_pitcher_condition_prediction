...
### ③ 원인 분해 (ablation) — 조건 분리

②는 크롭·확대를 동시에 적용해 어느 쪽이 효과였는지 갈리지 않았다.
조건을 분리해 같은 클립에 교차 적용했다. (클립 57de1baa, 4조건)

| 세그먼트 | 2D | 배드민턴3D | A 기준 | B 2배확대 | C 크롭+2배 | D 컷전체(240f) |
|---|---|---|---|---|---|---|
| 어깨너비 | 65% | 12% | 37% | 38% | **4%** | **1%** |
| 골반너비 | 61% | 10% | 43% | 43% | **4%** | **2%** |
| 몸통 | — | 10% | 26% | 25% | **4%** | 11% |
| 전완(R) | 69% | 32% | 30% | 29% | **10%** | **10%** |
| 상완(R) | 38% | 21% | 18% | 18% | **8%** | **2%** |
| 대퇴(R) | 38% | 21% | 16% | 15% | **2%** | **2%** |
| **중앙값** | | | **28%** | **27%** | **4%** | **2%** |

**크롭이 무엇을 바꿨나** (동일 클립 A vs C)

| 지표 | A 기준 | C 크롭 |
|---|---|---|
| 2D 신뢰도 | 0.751 | 0.738 (오히려 낮음) |
| 화면상 신장 | 339px | 344px (동일) |
| **프레임간 관절 이동(jitter)** | **8.66px** | **5.38px** |

→ 해상도·신뢰도는 그대로인데 **프레임 간 흔들림이 38% 감소**.
   크롭의 효과는 화질 향상이 아니라 **추적 안정화**였다.

### 결론

1. **원인은 피사체 크기가 아니라 추적 안정성이었다.**
   - 단순 2배 확대(B)는 효과 없음: 28% → 27%
   - 직전 프레임 기준 크롭(C)이 결정적: 27% → **4%**
   - 전체 프레임 추론에서는 매 프레임 검출기가 다른 사람을 잡거나 bbox가
     튀는데, 직전 프레임 기준 크롭이 대상을 고정한다. MotionBERT는 시퀀스
     모델이라 프레임 간 일관성에 민감하다.
   - ※ ①(더 큰 모델)이 실패한 것도 같은 이유. 2D 정확도는 병목이 아니었다.
2. **시퀀스 길이도 기여한다.** D(240프레임) 중앙값 2%, 어깨너비 1%.
   `CLIP_LEN=243`에 가까울수록 개선.
3. **조건만 맞추면 3D는 배드민턴 수준(4~12%)까지 나온다.**
   게이트 15%를 전 세그먼트가 통과. 던지는 팔(전완R 10%)도 포함.
4. **Stage 3은 전면 도입 가능하다.** 단 두 처리가 필수:
   ① 직전 프레임 기준 크롭 추적 (03번 `select_pitcher()` + PAD와 동일 발상)
   ② 컷 전체 또는 충분히 긴 구간 투입 (릴리스 앵커 80프레임보다 길게)
...
③의 C조건 4%는 클립 1개 결과였다. 6개 클립 전체에 C(크롭+2배, 80f)와
E(크롭+2배, 240f)를 적용해 재현성을 확인했다.

| 세그먼트 | 야구2D | 배드민턴3D | C (80f) | E (240f) | 판정 |
|---|---|---|---|---|---|
| 몸통 | — | 10% | **7%** | 10% | ✅ 통과 |
| 대퇴(R) | 38% | 21% | **7%** | 18% | ✅ 통과 |
| 상완(L) | 49% | 22% | **12%** | 21% | ✅ 통과 |
...
1. **중앙값 13%로 게이트는 통과하나, ③의 4%는 재현되지 않는다.**
   클립별로 4%~27%로 편차가 크다. 좋은 클립(4%)과 나쁜 클립(27%)이 공존하며,
   전자만 보고 판단하면 안 된다.
2. **통과는 3종뿐** (몸통 7%, 대퇴 7%, 상완L 12%). 체간(어깨·골반)은 17%로
   경계이고, **던지는 팔(상완R 27%)은 여전히 미달**이다.
   ③에서 "전완R 10%로 팔도 통과"라고 본 것은 클립 1개의 우연이었다.
3. **긴 시퀀스(E)의 효과는 세그먼트마다 갈린다.** 전완R 36%→22%,
   종아리 31%→22%로 팔다리는 개선되지만, 체간·대퇴는 오히려 나빠진다
   (대퇴 7%→18%). 240프레임에는 투구 외 구간이 섞여 들어오기 때문으로 추정.
4. **최종 판정: 부분 통과.** 3D는 2D(38~69%)보다 확실히 낫고 체간·하체는
   쓸 만하지만, 배드민턴 수준(10~12%)에는 못 미친다.
   → Stage 3은 **체간·하체 한정 도입**이 타당하다. 던지는 팔 각도
   (`arm_slot` 계열)는 3D로도 신뢰하기 어렵다.
5. **클립 편차의 원인 규명이 선행 과제.** 4%와 27%를 가르는 요인
   (시점·투수 크기·배경 혼잡도·모션블러)을 특정해야 전면 적용 여부를 정할 수 있다.
