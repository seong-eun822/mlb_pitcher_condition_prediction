...
- 수집 원본: 연도별 Parquet 파일로 저장 (`statcast_{year}.parquet`)
- 전처리 결과: DuckDB에 적재하여 SQL로 관리

### DuckDB 활용

5년치 MLB 전경기 데이터(수천만 행 규모)를 DuckDB를 통해 SQL로 핸들링한다.
...
SQL로 필터링 / 집계 / 구간 분리
    ↓
Feature Engineering 결과 → 모델 입력
```

활용 단계:
- **수집 후 적재**: 연도별 Parquet → DuckDB 테이블로 통합
- **선발투수 필터링**: `game_type`, `inning`, `pitcher` 기준 SQL 쿼리
- **기준값 집계**: 투수별 / 구종별 / 시즌별 평균 구속·spin rate 집계
- **X/Y 구간 분리**: `at_bat_number` 기준 구간 분리 쿼리
- **Y값 계산**: `woba_value`, `woba_denom` 기반 경기 단위 wOBA 집계

DuckDB를 선택한 이유:
- Parquet 파일을 변환 없이 직접 SQL로 조회 가능
- 수천만 행 규모의 분석 쿼리에 최적화된 columnar 엔진
- 로컬 / 코랩 환경 모두 설치 한 줄로 동작
...
- 대용량 SQL 핸들링: 5년치 수천만 행 규모 데이터를 DuckDB + SQL로 수집부터 전처리까지 일관 관리
