"""결측(NaN) 처리 전략 비교 실험. [LEGACY — 실행 불가, 기록 보존용]

delta feature는 해당 구종을 안 던진 경기에 NaN이 생긴다. 이 NaN을
어떻게 다룰지(그대로 두기 / 0 대체 / 컬럼 삭제)에 따른 성능을 비교한다.

⚠️ 이 파일은 **타겟이 wOBA였던 초기 시점**(`y_woba`)에 작성됐다.
   이후 타겟이 whiff%(`y_whiff`)로 바뀌었으므로 현재 feature 데이터로는
   그대로 실행되지 않는다.

   결과는 이미 확정됐고(5개 전략 모두 baseline이 최선 → 특별한 결측 처리
   없이 진행), 최종 파이프라인에서 이 모듈을 import하지 않는다.
   실험 이력을 남기기 위해 보존하는 것이므로 **수정하지 말 것.**
   재실행이 필요하면 `y_woba` → `y_whiff`로 바꾸고 결과를 새로 기록할 것.
"""
